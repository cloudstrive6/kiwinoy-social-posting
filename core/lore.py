...
GAME_LORE: dict[str, str] = {
    ...
    "spider-man2": (
        ...
        "TWO-HEROES RULE: this game stars BOTH Peter AND Miles. Judge each clip by its "
        "ACTUAL gameplay + story — don't assume solo OR duo. MANY fights ARE genuinely "
        "solo (e.g. Miles vs Martin Li / Mister Negative; Miles vs the SYMBIOTE-controlled "
        "Peter; a lone side mission), and those CAN be framed as one Spider-Man. But do NOT "
        "frame a CITY-WIDE / world-ending threat as one lone hero's solo problem: the "
        "SYMBIOTE INVASION of all New York is something BOTH Spider-Men fight, so never "
        "caption that wide-takeover footage as 'the ONE/ONLY/LONE Spider-Man' or 'the last "
        "hero'. When unsure whether a moment is solo or a team-up, just write \"Spider-Man\" "
        "and hook the ACTION or the threat, not the hero count.\n"
        ...
        "army of ARMED HUMAN mercenaries (guns, tech, jeeps) hunting the city's "
        "super-powered people. Primary EARLY villain — he has NOTHING to do with "
        "plants, thorns, tendrils, vines, or any organic city takeover.\n"
        ...
        "The late-game CITY TAKEOVER — red/black organic TENDRILS, spikes/'thorns', "
        "vines and veined growths spreading across BUILDINGS, streets and the SKYLINE "
        "— is the SYMBIOTE INVASION (the symbiote / Venom), NEVER Kraven. If a clip "
        "shows the city overgrown with tendrils/thorns, it is the symbiote invasion; "
        "do NOT attribute it to Kraven or call it 'Kraven's thorns'.\n"
        "SUITS (FYI so you correctly READ what's on screen — you need NOT name the suit in "
        "the caption). CRITICAL: every suit is RE-COLORABLE (color variants), so NEVER "
        "identify a suit by COLOR — use the EMBLEM shape, the panel/texture pattern, and the "
        "silhouette. A dark or green suit is NOT automatically the symbiote. Distinctive/story "
        "suits:\n"
        "  PETER — Advanced Suit 2.0: sleek, large WHITE spider emblem across chest+back with "
        "white spider-leg lines down the sides (his default hero look). SYMBIOTE suit: glossy "
        "ORGANIC black with a white spider that grows SPIKY/veined with tendrils + jagged "
        "white 'wound' lines as it takes over — THIS specific tendrilly look = symbiote Peter "
        "(a plain dark recolor of another suit is NOT). Anti-Venom: pale white/grey with black "
        "web-vein accents + a bold black spider (the 'cured' late-game look). Anti-Ock (returning "
        "from the 2018 game): GOLD/white hexagonal tech PANELS on the chest — the user's example "
        "'green Anti-Ock' is this suit RE-COLORED green (NOT the symbiote).\n"
        "  MILES — red/black spider suits; his signature tell is glowing YELLOW bio-electric "
        "'Venom' energy crackling around him (and purple camouflage shimmer), NOT the suit "
        "color. Flashier variants exist (e.g. purple/pink Programmable Matter).\n"
        "  This is a CURATED set of the distinctive + story suits, not the full ~100-suit "
        "catalog. Still follow the NAMING RULE: unless it's the tendrilly SYMBIOTE (=Peter) or "
        "Miles's YELLOW bio-electric zaps (=Miles), just say \"Spider-Man\"."
    ),
}
...
GAME_LORE["wolverine"] = (
    "MARVEL'S WOLVERINE (Insomniac Games, PS5, released 2026) — a MATURE-rated, brutal, "
    "STORY-DRIVEN third-person action game (NOT open world). SINGLE playable hero: you "
    "ALWAYS play LOGAN / James Howlett, aka WOLVERINE (voiced by Liam McIntyre). Name him "
    "confidently as 'Wolverine' or 'Logan'.\n"
    "WHO HE IS: a gruff, world-weary mutant — three retractable ADAMANTIUM claws that snap "
    "from the BACK of each hand, an adamantium-laced skeleton, a rapid HEALING FACTOR, "
    "heightened animal senses, and a feral BERSERKER RAGE. Combat is visceral and bloody. "
    "He is haunted by fractured MEMORIES of his past.\n"
    "PREMISE: about 3 years after Logan ABANDONED his old black-ops unit TEAM X, he "
    "reunites with them to investigate the abduction of vulnerable mutants by billionaire "
    "industrialist BOLIVAR TRASK and his cybernetic militia the REAVERS, and to stop the "
    "experiments of MISTER SINISTER (Dr. Nathaniel Essex), who created Team X. Themes: "
    "memory, betrayal, redemption. Globe-trots across Canada, Vancouver, Japan, Madripoor.\n"
    "ALLIES:\n"
    "- JEAN GREY: Logan's ally — telekinetic/telepathic combat support and an emotional "
    "tie to his past. NICKNAME: Logan calls Jean 'RED' (his long-standing name for her). A "
    "line like \"I've never seen you like this, Red\" is Logan talking to JEAN, an ALLY at his "
    "side — NEVER Omega Red (who works for Trask). Still name Jean only when the scene "
    "shows her; otherwise say 'his ally'.\n"
    "- TEAM X (Logan's former unit): members include SABRETOOTH (Victor Creed — a savage "
    "clawed brawler much like Logan; he feels abandoned by Logan and grows UNSTABLE) and "
    "MYSTIQUE (blue shape-shifter). CRITICAL: Sabretooth & Mystique are TEAM X here, so a "
    "clip of Logan alongside them is NOT automatically a fight — never auto-frame "
    "Sabretooth/Mystique as straight enemies; judge the actual clip.\n"
    "- CALLISTO: a rescued mutant. SUNFIRE (Shiro Yoshida): a fire/plasma mutant ally in "
    "the late game.\n"
    "ANTAGONISTS / BOSSES: BOLIVAR TRASK + the REAVERS (cyborg mercenaries) abduct mutants "
    "early; OMEGA RED (Arkady Rossovich — a super-soldier with metal tentacle-coils; a "
    "FORMER Team X member who now SIDES WITH TRASK and hunts his old Team X colleagues; the "
    "FIRST major boss — so a clip of Omega Red among Trask's Reavers is him with his OWN "
    "side, NOT threatening them); LADY DEATHSTRIKE (adamantium-clawed, tied to the HAND "
    "ninja cult); MASTER MOLD (a giant Sentinel-making machine).\n"
    "SENTINELS vs REAVERS (don't mix them up): a tall PURPLE/BLUE armoured ROBOT with a red "
    "visor/eyes and a glowing triangle emblem is a SENTINEL (subtitles label it e.g. 'MK2 "
    "Sentinel:') — a machine, never a 'Reaver brute'. REAVERS are Trask's human cyborg "
    "soldiers/mercs (subtitles 'Reaver Squad Leader:', 'Reaver Master Gunner:').\n"
    "KEY TWISTS (so you correctly READ a fight clip): (1) SABRETOOTH is Logan's Team X "
    "ALLY for MOST of the game — the two fight SIDE BY SIDE against Trask's Reavers and "
    "soldiers. DEFAULT TO ALLIES: a clip where Logan AND Sabretooth are both attacking "
    "OTHER enemies (Reavers, mercs, Trask's men) is them fighting TOGETHER — caption it as "
    "a team-up, and NEVER as a 'betrayal fight' or 'Wolverine vs Sabretooth'. Only ONE "
    "LATE-GAME duel has them turn on EACH OTHER; call it a rival/betrayal fight ONLY when "
    "the clip UNMISTAKABLY shows Sabretooth and Logan striking ONE ANOTHER 1v1 with no "
    "other enemies around. In ANY doubt, they are allies fighting a common foe. (2) "
    "NATHANIEL ESSEX is, for MOST of the game, the man Team X set out to RESCUE and a "
    "father figure to Logan & Victor — treat him as an ALLY / the person they are trying "
    "to SAVE, NOT an enemy. Only LATE does he turn: he is revealed as MISTER SINISTER and "
    "seizes the HELIX ENGINE to trigger dormant X-genes worldwide and wipe out non-mutants, "
    "with the final battle in TOKYO. Frame Essex as a VILLAIN ONLY when the clip is clearly "
    "that late-game turn (him enacting the plan, or Logan fighting him); any earlier Essex "
    "clip is the rescue / ally phase — never pre-label him evil. APOCALYPSE (an ancient "
    "mutant) is only TEASED late — do NOT call him the final boss.\n"
    "NAMING RULE: name a character ONLY when the clip's VISUALS or SUBTITLES unmistakably "
    "show them (an on-screen 'OMEGA RED:' subtitle, Sabretooth's feral look, Mystique's "
    "blue skin, etc.). Otherwise say 'enemies' / 'a foe' / the Reavers. Do NOT invent "
    "characters or specific plot beats you can't see.\n"
    "DIALOGUE & RELATIONSHIP RULE (CRITICAL — this has caused wrong captions): do NOT "
    "assert WHO a character is speaking to, threatening, allied with, or WHY (their target "
    "or motive) unless the clip's SUBTITLES/VISUALS make it explicit. A villain speaking a "
    "threat is often addressing his OWN side, not the hero — never assume a line is aimed "
    "at Wolverine or 'Team X', and never tag a motive like 'over Wolverine', unless the "
    "scene shows it. Describe the on-screen ACTION and only the relationships you can "
    "actually see; when unsure, hook the action itself, not who/why.\n"
    "TIMELINE: Logan is ALREADY a fully-powered Wolverine with adamantium claws — never "
    "write it's 'before the adamantium' or an origin unless the clip clearly depicts a "
    "memory/flashback saying so.\n"
    "DON'T FABRICATE: no invented quotes or events. When unsure, hook the raw brutality — "
    "the claws, the healing, the rage."
)
