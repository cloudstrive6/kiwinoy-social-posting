...
def main() -> int:
    ...
    # Upload to B2 drafts/threads/ under a self-describing name + ping Telegram.
    fname = content.draft_filename("threads", game, fmt, content.footage_keyword(caption, locals().get("hook", "")))
    la = CONFIG.raw().get("longform_archive", {}) or {}
    remote = str(la.get("remote", "kgb2"))
    key = f"drafts/threads/{fname}"
